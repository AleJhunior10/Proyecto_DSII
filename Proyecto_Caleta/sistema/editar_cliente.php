...
if (!empty($_POST)) {
  ...
  if (empty($_POST['nombre']) || empty($_POST['telefono']) || empty($_POST['direccion'])) {
    ...
    if (is_numeric($ci_nit) and $ci_nit != 0) {
      ...
      $query = mysqli_query($conexion, "SELECT * FROM cliente where (ci_nit = '$ci_nit' AND idcliente != $idcliente)");
      $result = mysqli_fetch_array($query);
      $resul = mysqli_num_rows($query);
    }

    if ($resul >= 1) {
      $alert = '<p class"error">El ci_nit ya existe</p>';
    } else {
      if ($ci_nit == '') {
        $ci_nit = 0;
      }
      $sql_update = mysqli_query($conexion, "UPDATE cliente SET ci_nit = $ci_nit, nombre = '$nombre' , telefono = '$telefono', direccion = '$direccion' WHERE idcliente = $idcliente");
            ...
            <div class="col-lg-6 m-auto">
              ...
              <form class="" action="" method="post">
                ...
                <div class="form-group">
                  <label for="ci_nit">ci_nit</label>
                  <input type="number" placeholder="Ingrese ci_nit" name="ci_nit" id="ci_nit" class="form-control" value="<?php echo $ci_nit; ?>">
                </div>
                <div class="form-group">
