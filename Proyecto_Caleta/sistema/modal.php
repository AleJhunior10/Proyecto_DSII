...
if ($_POST['action'] == 'searchCliente') {
  if (!empty($_POST['cliente'])) {
    $ci_nit = $_POST['cliente'];

    $query = mysqli_query($conexion, "SELECT * FROM cliente WHERE ci_nit LIKE '$ci_nit'");
    mysqli_close($conexion);
    $result = mysqli_num_rows($query);
    $data = '';
    if ($result > 0) {
...
if ($_POST['action'] == 'addCliente') {
  $ci_nit = $_POST['dni_cliente'];
  $nomnre = $_POST['nom_cliente'];
  $telefono = $_POST['tel_cliente'];
  $direccion = $_POST['dir_cliente'];
  $usuario_id = $_SESSION['idUser'];

  $query_insert = mysqli_query($conexion, "INSERT INTO cliente(ci_nit, nombre, telefono, direccion, usuario_id) VALUES ('$ci_nit','$nomnre','$telefono','$direccion','$usuario_id')");
  if ($query_insert) {
